...
GAN_seqLen  = 128;
GAN_zDim    = 16;
GAN_cDim    = 2;      % Info code dim (continuous in [0,1]), used to induce diverse jammer "types" without hard-coded templates
GAN_infoLambda = 1.0; % InfoGAN-style code-predict loss weight (higher -> easier to classify)
GAN_trainIters = 250;
GAN_modelFile  = 'InfoGAN_Jammer_R2021a.mat';
...
if EnableWorstCaseSearch
    [netG, ~, ~] = trainOrLoadJammerGAN(GAN_seqLen, GAN_zDim, GAN_cDim, GAN_trainIters, GAN_modelFile, GAN_infoLambda);
 
    nvars = GAN_zDim + GAN_cDim + 1;  % [z ; c(info) ; JamScale_dB]
    lb = [z_lb*ones(GAN_zDim,1); zeros(GAN_cDim,1); JamScale_lb_dB];
    ub = [z_ub*ones(GAN_zDim,1); ones(GAN_cDim,1); JamScale_ub_dB];
 
    obj = @(x) worstCaseObjective( ...
        x, netG, GAN_seqLen, GAN_zDim, GAN_cDim, ...
        numSteps, sample_time, ...
        visU_all, visG_all, ...
        azU, elU, rU, rrU, ...
        elG, rG, ...
    ...
    try
        ...
        fprintf('[警告] GA失败：%s\n', ME.message);
        xBest = [zeros(GAN_zDim,1); 0.5*ones(GAN_cDim,1); 20];
    end
 
    zBest = xBest(1:GAN_zDim);
    cBest = xBest(GAN_zDim + (1:GAN_cDim));
    JamScaleBest_dB = xBest(end);

    jamAggWorst = genJamAggFromG(netG, zBest, cBest, GAN_seqLen, numSteps);
else
    JamScaleBest_dB = 0;
...
% Store GA/GAN "info code" (continuous control variables in [0,1]) for downstream sampler/classifier
if exist('cBest','var')
    simWorst.JamInfoCode = cBest(:).';
else
    simWorst.JamInfoCode = 0.5*ones(1, GAN_cDim);
end
...
if EnableInterfClassifier
    ...
    % NEW (Route-1): remove hard-coded jammer/CCI spectral "type" selection.
    % Use the GA/GAN-discovered continuous info code (in [0,1]) to drive waveform morphology.
    if isfield(simWorst,'JamInfoCode')
        SamplerCfg.InfoCode = simWorst.JamInfoCode;
    end
...
function f = worstCaseObjective( ...
    ...
    jamAgg = genJamAggFromG(netG, z, cCode, seqLen, numSteps);
